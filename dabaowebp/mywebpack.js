...
class Compiler {
  constructor(webpackOptions) {
    this.options = webpackOptions; //储存配置信息
    this.hooks = {
      run: new SyncHook(), //编译开始的时候触发的钩子
      done: new SyncHook(), //编译结束的时候触发是钩子
    };
  }
  compile(callback) {
...
class Compilation {
  constructor(webpackOptions) {
    ...
    this.modules = []; //编译生成的全部模块
    this.chunks = []; //本次编译产出的所有代码块，入口模块和依赖的模块打包在一起为代码
