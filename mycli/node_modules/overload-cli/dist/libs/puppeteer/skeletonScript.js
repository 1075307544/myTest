...
window.SkeletonScript = (function () {
    ...
    function singleBlockHandler(element) {
        ...
        const { display, float, flex, position, left, top, right, bottom, width, height, padding, margin, transform, borderRadius, overflow, } = window.getComputedStyle(element);
    ...
    function textHandler(element) {
        // 1. 初始化各类变量
        let _a = window.getComputedStyle(element), { fontSize, lineHeight, paddingTop, paddingBottom, textAlign } = _a, rest = __rest(_a, ["fontSize", "lineHeight", "paddingTop", "paddingBottom", "textAlign"]);
        const { width, height } = element.getBoundingClientRect();
    ...
    function genSkeleton(options = {}) {
      ...
      border: none !important;
        ...
        // 5. 删除测量 text 的 dom
        if (offScreenWrapElement)
            document.body.removeChild(offScreenWrapElement);
